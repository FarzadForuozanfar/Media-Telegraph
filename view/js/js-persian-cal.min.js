...
AMIB.persianCalendar = function (b, a) {
    var c = { inputID: "", extraInputID: "", extraInputFormat: "JD", initialDate: "", defaultDate: "", autoCreateButton: !0, divPickerClassName: "picker", btnClassName: "pcalBtn", onchange: !1 }, d; this.getOption = function (a) { return c[a] }; this.oldDocumentOnClick = !1; this.enabled = !0; this.inputID = b; this.input = A$(b); for (d in a) d in c ? c[d] = a[d] : alert(d.toString() + " option is not supported"); c.defaultDate instanceof Array || (c.defaultDate = isPersianDate(c.defaultDate)); d = this.input; if (c.autoCreateButton) {
        var e =
...
AMIB.persianCalendar.prototype.showHidePicker = function (b) {
    var a = this.inputID + "_pDatePicker", c; if (c = A$(a)) { if (c.parentNode.removeChild(c), document.onclick = this.oldDocumentOnClick, lastOpenPDatePicker = this.oldDocumentOnClick = !1, b = A$(lastOpenPDateMonthYearID)) b.parentNode.removeChild(b), lastOpenPDateMonthYearID = !1 } else if (!0 === this.enabled) {
        var d = this.input; c = cE("div"); c.id = a; c.className = this.getOption("divPickerClassName"); this.createDateTable(c, d.value, d.value); document.body.appendChild(c); this.setMenuPos(c,
...
AMIB.persianCalendar.prototype.createDateTable = function (b, a, c) {
    var d = cE("table"), e, g, f, h, k, j; g = this.getOption("defaultDate"); j = new Date; f = gregorian_to_jd(j.getFullYear(), j.getMonth() + 1, j.getDate()); j = jd_to_persian(f); (a = isPersianDate(a)) ? a = persian_to_jd(a[0], a[1], a[2]) : g ? (a = persian_to_jd(g[0], g[1], g[2]), c = g.join("/")) : a = f; c = isPersianDate(c); a = jd_to_persian(a); var l = Math.floor(persian_to_jd(a[0], a[1], 1) + 3) % 7; e = d.insertRow(-1); g = e.insertCell(-1); g.className = "pickerHead"; g.colSpan = 7; e = cE("div"); e.className =
    ...
    b.appendChild(d)
}; AMIB.persianCalendar.prototype.changePickerDate = function (b, a, c) { var d; if (d = A$(this.inputID + "_pDatePicker")) b || (b = window.event), stopPropg(b), b = a.toString() + "/" + c.toString() + "/1", this.createDateTable(d, b, this.input.value) };
...
AMIB.persianCalendar.prototype.showYearPicker = function (b, a, c) {
    var d, e, g; if (A$(this.inputID + "_pDatePicker")) {
        b || (b = window.event); (e = A$(lastOpenPDateMonthYearID)) && e.parentNode.removeChild(e); stopPropg(b); e = cE("div"); e.className = "monthYearPicker"; e.id = lastOpenPDateMonthYearID = this.inputID + "monthYearPicker"; for (d = c - 5; d <= c + 5; ++d)g = cE("a"), g.href = "#", g.appendChild(cTN(enDigitsToFa(d))), d == c && (g.className = "selected"), g.onclick = function (a, b, c) { return function (d) { a.monthYearPickerClick(d, b, c); return !1 } }(this,
            a, d), e.appendChild(g); document.body.appendChild(e); this.setMenuPos(e, b.target || b.srcElement); e.style.zIndex = ++pDatePickerZIndex
    }
}; AMIB.persianCalendar.prototype.setMenuPos = function (b, a) { var c = getOffset(a); b.style.top = 0 > c.top - b.offsetHeight || getScrollTop() + getViewPortHeight() > c.top + a.offsetHeight + b.offsetHeight ? c.top + a.offsetHeight + "px" : c.top - b.offsetHeight + "px"; c = getOffset(a); b.style.left = c.left + a.offsetWidth - b.offsetWidth + "px" };
